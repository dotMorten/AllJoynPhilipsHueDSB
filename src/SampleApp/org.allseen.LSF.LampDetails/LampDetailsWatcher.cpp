...
LampDetailsWatcher::LampDetailsWatcher(AllJoynBusAttachment^ busAttachment) :
...
LampDetailsWatcher::~LampDetailsWatcher()
{
    UnregisterFromBus();
}

void LampDetailsWatcher::UnregisterFromBus()
{
    if (nullptr != m_aboutListener)
    {
        PCSTR interfaces[] = { "org.allseen.LSF.LampDetails" };
...
void LampDetailsWatcher::OnAnnounce(
    _In_ PCSTR name,
    ...
    _In_ const alljoyn_msgarg aboutDataArg)
{
    ...
    if (alljoyn_aboutobjectdescription_hasinterface(objectDescription, "org.allseen.LSF.LampDetails"))
    {
        AllJoynServiceInfo^ args = ref new AllJoynServiceInfo(
            AllJoynHelpers::MultibyteToPlatformString(name),
            AllJoynHelpers::GetObjectPath(objectDescription, "org.allseen.LSF.LampDetails"),
            port);
        Added(this, args);
    }
...
void LampDetailsWatcher::BusAttachmentStateChanged(_In_ AllJoynBusAttachment^ sender, _In_ AllJoynBusAttachmentStateChangedEventArgs^ args)
{
    if (args->State == AllJoynBusAttachmentState::Connected)
    {
        alljoyn_aboutlistener_callback callbacks = 
        {
            AllJoynHelpers::AnnounceHandler<LampDetailsWatcher>
        };
...
void LampDetailsWatcher::Start()
{
    ...
    int32 result = AllJoynHelpers::CreateInterfaces(m_busAttachment, c_LampDetailsIntrospectionXml);
    ...
    m_busAttachmentStateChangedToken = m_busAttachment->StateChanged += ref new TypedEventHandler<AllJoynBusAttachment^, AllJoynBusAttachmentStateChangedEventArgs^>(this, &LampDetailsWatcher::BusAttachmentStateChanged);
    m_busAttachment->Connect();
}

void LampDetailsWatcher::Stop()
{
    StopInternal(AllJoynStatus::Ok);
}

void LampDetailsWatcher::StopInternal(int32 status)
{
    UnregisterFromBus();
