...
LampServiceWatcher::LampServiceWatcher(AllJoynBusAttachment^ busAttachment) :
...
LampServiceWatcher::~LampServiceWatcher()
{
    UnregisterFromBus();
}

void LampServiceWatcher::UnregisterFromBus()
{
    if (nullptr != m_aboutListener)
    {
        PCSTR interfaces[] = { "org.allseen.LSF.LampService" };
...
void LampServiceWatcher::OnAnnounce(
    _In_ PCSTR name,
    ...
    _In_ const alljoyn_msgarg aboutDataArg)
{
    ...
    if (alljoyn_aboutobjectdescription_hasinterface(objectDescription, "org.allseen.LSF.LampService"))
    {
        AllJoynServiceInfo^ args = ref new AllJoynServiceInfo(
            AllJoynHelpers::MultibyteToPlatformString(name),
            AllJoynHelpers::GetObjectPath(objectDescription, "org.allseen.LSF.LampService"),
            port);
        Added(this, args);
    }
...
void LampServiceWatcher::BusAttachmentStateChanged(_In_ AllJoynBusAttachment^ sender, _In_ AllJoynBusAttachmentStateChangedEventArgs^ args)
{
    if (args->State == AllJoynBusAttachmentState::Connected)
    {
        alljoyn_aboutlistener_callback callbacks = 
        {
            AllJoynHelpers::AnnounceHandler<LampServiceWatcher>
        };
...
void LampServiceWatcher::Start()
{
    ...
    int32 result = AllJoynHelpers::CreateInterfaces(m_busAttachment, c_LampServiceIntrospectionXml);
    ...
    m_busAttachmentStateChangedToken = m_busAttachment->StateChanged += ref new TypedEventHandler<AllJoynBusAttachment^, AllJoynBusAttachmentStateChangedEventArgs^>(this, &LampServiceWatcher::BusAttachmentStateChanged);
    m_busAttachment->Connect();
}

void LampServiceWatcher::Stop()
{
    StopInternal(AllJoynStatus::Ok);
}

void LampServiceWatcher::StopInternal(int32 status)
{
    UnregisterFromBus();
