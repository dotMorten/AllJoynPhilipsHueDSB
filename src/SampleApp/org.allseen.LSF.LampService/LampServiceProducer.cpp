...
void LampServiceProducer::CallClearLampFaultHandler(_Inout_ alljoyn_busobject busObject, _In_ alljoyn_message message)
{
    ...
    if (producer->Service != nullptr)
    {
        ...
        LampServiceClearLampFaultResult^ result = create_task(producer->Service->ClearLampFaultAsync(callInfo, inputArg0)).get();
        create_task([](){}).then([=]
        {
            int32 status;
...
PCSTR org::allseen::LSF::LampService::c_LampServiceIntrospectionXml = "<interface name=\"org.allseen.LSF.LampService\">"
