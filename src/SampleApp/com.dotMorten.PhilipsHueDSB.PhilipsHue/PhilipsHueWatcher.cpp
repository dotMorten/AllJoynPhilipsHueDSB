...
PhilipsHueWatcher::PhilipsHueWatcher(AllJoynBusAttachment^ busAttachment) :
...
PhilipsHueWatcher::~PhilipsHueWatcher()
{
    UnregisterFromBus();
}

void PhilipsHueWatcher::UnregisterFromBus()
{
    if (nullptr != m_aboutListener)
    {
        PCSTR interfaces[] = { "com.dotMorten.PhilipsHueDSB.PhilipsHue" };
...
void PhilipsHueWatcher::OnAnnounce(
    _In_ PCSTR name,
    ...
    _In_ const alljoyn_msgarg aboutDataArg)
{
    ...
    if (alljoyn_aboutobjectdescription_hasinterface(objectDescription, "com.dotMorten.PhilipsHueDSB.PhilipsHue"))
    {
        AllJoynServiceInfo^ args = ref new AllJoynServiceInfo(
            AllJoynHelpers::MultibyteToPlatformString(name),
            AllJoynHelpers::GetObjectPath(objectDescription, "com.dotMorten.PhilipsHueDSB.PhilipsHue"),
            port);
        Added(this, args);
    }
...
void PhilipsHueWatcher::BusAttachmentStateChanged(_In_ AllJoynBusAttachment^ sender, _In_ AllJoynBusAttachmentStateChangedEventArgs^ args)
{
    if (args->State == AllJoynBusAttachmentState::Connected)
    {
        alljoyn_aboutlistener_callback callbacks = 
        {
            AllJoynHelpers::AnnounceHandler<PhilipsHueWatcher>
        };
...
void PhilipsHueWatcher::Start()
{
    ...
    int32 result = AllJoynHelpers::CreateInterfaces(m_busAttachment, c_PhilipsHueIntrospectionXml);
    ...
    m_busAttachmentStateChangedToken = m_busAttachment->StateChanged += ref new TypedEventHandler<AllJoynBusAttachment^, AllJoynBusAttachmentStateChangedEventArgs^>(this, &PhilipsHueWatcher::BusAttachmentStateChanged);
    m_busAttachment->Connect();
}

void PhilipsHueWatcher::Stop()
{
    StopInternal(AllJoynStatus::Ok);
}

void PhilipsHueWatcher::StopInternal(int32 status)
{
    UnregisterFromBus();
