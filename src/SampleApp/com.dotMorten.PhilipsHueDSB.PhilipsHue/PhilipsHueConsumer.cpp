...
std::map<alljoyn_interfacedescription, WeakReference*> PhilipsHueConsumer::SourceInterfaces;

PhilipsHueConsumer::PhilipsHueConsumer(AllJoynBusAttachment^ busAttachment)
...
PhilipsHueConsumer::~PhilipsHueConsumer()
{
...
void PhilipsHueConsumer::OnSessionLost(_In_ alljoyn_sessionid sessionId, _In_ alljoyn_sessionlostreason reason)
{
...
void PhilipsHueConsumer::OnSessionMemberAdded(_In_ alljoyn_sessionid sessionId, _In_ PCSTR uniqueName)
{
    if (sessionId == m_sessionId)
    {
...
void PhilipsHueConsumer::OnSessionMemberRemoved(_In_ alljoyn_sessionid sessionId, _In_ PCSTR uniqueName)
{
    if (sessionId == m_sessionId)
    {
...
QStatus PhilipsHueConsumer::AddSignalHandler(_In_ alljoyn_busattachment busAttachment, _In_ alljoyn_interfacedescription interfaceDescription, _In_ PCSTR methodName, _In_ alljoyn_messagereceiver_signalhandler_ptr handler)
{
...
IAsyncOperation<PhilipsHueJoinSessionResult^>^ PhilipsHueConsumer::JoinSessionAsync(
    _In_ AllJoynServiceInfo^ serviceInfo, _Inout_ PhilipsHueWatcher^ watcher)
{
    return create_async([serviceInfo, watcher]() -> PhilipsHueJoinSessionResult^
    {
        auto result = ref new PhilipsHueJoinSessionResult();
        result->Consumer = ref new PhilipsHueConsumer(watcher->BusAttachment);
...
IAsyncOperation<PhilipsHueGetIsLinkedResult^>^ PhilipsHueConsumer::GetIsLinkedAsync()
{
    return create_async([this]() -> PhilipsHueGetIsLinkedResult^
    {
        ...
        result->IsLinked = getContext.GetValue();
        return result;
    });
}

void PhilipsHueConsumer::OnPropertyChanged(_In_ alljoyn_proxybusobject obj, _In_ PCSTR interfaceName, _In_ const alljoyn_msgarg changed, _In_ const alljoyn_msgarg invalidated)
{
    UNREFERENCED_PARAMETER(obj);
    ...
    for (size_t i = 0; i < changedPropertyCount; i++)
    {
        ...
        if (strcmp("IsLinked", propertyName) == 0)
        {
            bool argument;
            (void)TypeConversionHelpers::GetAllJoynMessageArg(propertyValue, "b", &argument);
            IsLinkedChanged(this, (Platform::Object^)argument);
        }
    }
}

int32 PhilipsHueConsumer::JoinSession(_In_ AllJoynServiceInfo^ serviceInfo)
{
    alljoyn_sessionlistener_callbacks callbacks =
    {
        AllJoynHelpers::SessionLostHandler<PhilipsHueConsumer>,
        AllJoynHelpers::SessionMemberAddedHandler<PhilipsHueConsumer>,
        AllJoynHelpers::SessionMemberRemovedHandler<PhilipsHueConsumer>
    };
    ...
    RETURN_IF_QSTATUS_ERROR(alljoyn_proxybusobject_registerpropertieschangedlistener(
        ...
        AllJoynHelpers::PropertyChangedHandler<PhilipsHueConsumer>, 
        m_weak));


    alljoyn_interfacedescription description = alljoyn_busattachment_getinterface(m_nativeBusAttachment, "com.dotMorten.PhilipsHueDSB.PhilipsHue");
