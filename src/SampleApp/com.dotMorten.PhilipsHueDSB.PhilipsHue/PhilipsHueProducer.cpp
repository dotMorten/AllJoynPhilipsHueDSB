...
using namespace com::dotMorten::PhilipsHueDSB::PhilipsHue;

std::map<alljoyn_busobject, WeakReference*> PhilipsHueProducer::SourceObjects;
std::map<alljoyn_interfacedescription, WeakReference*> PhilipsHueProducer::SourceInterfaces;

PhilipsHueProducer::PhilipsHueProducer(AllJoynBusAttachment^ busAttachment)
    ...
    m_sessionId(0)
{
    ...
    m_signals = ref new PhilipsHueSignals();
    m_busAttachmentStateChangedToken.Value = 0;
}

PhilipsHueProducer::~PhilipsHueProducer()
{
    UnregisterFromBus();
    delete m_weak;
}

void PhilipsHueProducer::UnregisterFromBus()
{
...
bool PhilipsHueProducer::OnAcceptSessionJoiner(_In_ alljoyn_sessionport sessionPort, _In_ PCSTR joiner, _In_ const alljoyn_sessionopts opts)
{
...
void PhilipsHueProducer::OnSessionJoined(_In_ alljoyn_sessionport sessionPort, _In_ alljoyn_sessionid id, _In_ PCSTR joiner)
{
    ...
    {
        AllJoynHelpers::SessionLostHandler<PhilipsHueProducer>,
        AllJoynHelpers::SessionMemberAddedHandler<PhilipsHueProducer>,
        AllJoynHelpers::SessionMemberRemovedHandler<PhilipsHueProducer>
    };
...
void PhilipsHueProducer::OnSessionLost(_In_ alljoyn_sessionid sessionId, _In_ alljoyn_sessionlostreason reason)
{
...
void PhilipsHueProducer::OnSessionMemberAdded(_In_ alljoyn_sessionid sessionId, _In_ PCSTR uniqueName)
{
    if (sessionId == m_sessionId)
    {
...
void PhilipsHueProducer::OnSessionMemberRemoved(_In_ alljoyn_sessionid sessionId, _In_ PCSTR uniqueName)
{
    if (sessionId == m_sessionId)
    {
...
void PhilipsHueProducer::BusAttachmentStateChanged(_In_ AllJoynBusAttachment^ sender, _In_ AllJoynBusAttachmentStateChangedEventArgs^ args)
{
    if (args->State == AllJoynBusAttachmentState::Connected)
    {   
        QStatus result = AllJoynHelpers::CreateProducerSession<PhilipsHueProducer>(m_busAttachment, m_weak);
        if (ER_OK != result)
        {
            StopInternal(result);
            return;
...
QStatus PhilipsHueProducer::AddMethodHandler(_In_ alljoyn_interfacedescription interfaceDescription, _In_ PCSTR methodName, _In_ alljoyn_messagereceiver_methodhandler_ptr handler)
{
...
QStatus PhilipsHueProducer::AddSignalHandler(_In_ alljoyn_busattachment busAttachment, _In_ alljoyn_interfacedescription interfaceDescription, _In_ PCSTR methodName, _In_ alljoyn_messagereceiver_signalhandler_ptr handler)
{
    alljoyn_interfacedescription_member member;
    if (!alljoyn_interfacedescription_getmember(interfaceDescription, methodName, &member))
    {
...
QStatus PhilipsHueProducer::OnPropertyGet(_In_ PCSTR interfaceName, _In_ PCSTR propertyName, _Inout_ alljoyn_msgarg value)
{
    UNREFERENCED_PARAMETER(interfaceName);

    if (0 == strcmp(propertyName, "IsLinked"))
    {
        auto task = create_task(Service->GetIsLinkedAsync(nullptr));
        ...
        return create_task([](){}).then([=]() -> QStatus
        {
            ...
            return static_cast<QStatus>(TypeConversionHelpers::SetAllJoynMessageArg(value, "b", result->IsLinked));
...
QStatus PhilipsHueProducer::OnPropertySet(_In_ PCSTR interfaceName, _In_ PCSTR propertyName, _In_ alljoyn_msgarg value)
{
    UNREFERENCED_PARAMETER(interfaceName);
...
void PhilipsHueProducer::EmitIsLinkedChanged()
{
    create_task([&]
    {
        alljoyn_msgarg value = alljoyn_msgarg_create();
        OnPropertyGet("com.dotMorten.PhilipsHueDSB.PhilipsHue", "IsLinked", value);

        alljoyn_busobject_emitpropertychanged(
            m_busObject,
            "com.dotMorten.PhilipsHueDSB.PhilipsHue",
            "IsLinked",
            value,
            m_sessionId);
...
void PhilipsHueProducer::Start()
{
    ...
    QStatus result = AllJoynHelpers::CreateInterfaces(m_busAttachment, c_PhilipsHueIntrospectionXml);
    if (result != ER_OK)
    {
        StopInternal(result);
        return;
    ...
    result = AllJoynHelpers::CreateBusObject<PhilipsHueProducer>(m_weak);
    if (result != ER_OK)
    {
        StopInternal(result);
        return;
    ...
    alljoyn_interfacedescription interfaceDescription = alljoyn_busattachment_getinterface(AllJoynHelpers::GetInternalBusAttachment(m_busAttachment), "com.dotMorten.PhilipsHueDSB.PhilipsHue");
    ...
    m_busAttachmentStateChangedToken = m_busAttachment->StateChanged += ref new TypedEventHandler<AllJoynBusAttachment^,AllJoynBusAttachmentStateChangedEventArgs^>(this, &PhilipsHueProducer::BusAttachmentStateChanged);
    m_busAttachment->Connect();
}

void PhilipsHueProducer::Stop()
{
    StopInternal(AllJoynStatus::Ok);
}

void PhilipsHueProducer::StopInternal(int32 status)
{
    UnregisterFromBus();
    Stopped(this, ref new AllJoynProducerStoppedEventArgs(status));
}

int32 PhilipsHueProducer::RemoveMemberFromSession(_In_ String^ uniqueName)
{
...
PCSTR com::dotMorten::PhilipsHueDSB::PhilipsHue::c_PhilipsHueIntrospectionXml = "<interface name=\"com.dotMorten.PhilipsHueDSB.PhilipsHue\">"
"  <property name=\"IsLinked\" type=\"b\" access=\"read\">"
