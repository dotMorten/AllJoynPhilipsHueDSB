...
void MainInterfaceProducer::CallLinkHandler(_Inout_ alljoyn_busobject busObject, _In_ alljoyn_message message)
{
    ...
    if (producer->Service != nullptr)
    {
        ...
        MainInterfaceLinkResult^ result = create_task(producer->Service->LinkAsync(callInfo)).get();
        create_task([](){}).then([=]
        {
            int32 status;
            ...
            status = TypeConversionHelpers::SetAllJoynMessageArg(alljoyn_msgarg_array_element(outputs, 0), "s", result->Result);
            if (AllJoynStatus::Ok != status)
            {
                alljoyn_busobject_methodreply_status(busObject, message, static_cast<QStatus>(status));
...
void MainInterfaceProducer::Start()
{
    ...
    alljoyn_interfacedescription interfaceDescription = alljoyn_busattachment_getinterface(AllJoynHelpers::GetInternalBusAttachment(m_busAttachment), "com.dotMorten.PhilipsHueDSB.PhilipsHue.MainInterface");
    ...
    result = AddMethodHandler(
        ...
        "Link", 
        [](alljoyn_busobject busObject, const alljoyn_interfacedescription_member* member, alljoyn_message message) { UNREFERENCED_PARAMETER(member); CallLinkHandler(busObject, message); });
    if (result != ER_OK)
    {
        StopInternal(result);
        return;
...
PCSTR com::dotMorten::PhilipsHueDSB::PhilipsHue::MainInterface::c_MainInterfaceIntrospectionXml = "<interface name=\"com.dotMorten.PhilipsHueDSB.PhilipsHue.MainInterface\">"
"  <method name=\"Link\">"
