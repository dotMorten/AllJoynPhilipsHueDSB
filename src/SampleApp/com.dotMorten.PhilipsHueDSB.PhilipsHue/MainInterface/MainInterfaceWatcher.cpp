...
using namespace com::dotMorten::PhilipsHueDSB::PhilipsHue::MainInterface;

MainInterfaceWatcher::MainInterfaceWatcher(AllJoynBusAttachment^ busAttachment) :
...
MainInterfaceWatcher::~MainInterfaceWatcher()
{
    UnregisterFromBus();
}

void MainInterfaceWatcher::UnregisterFromBus()
{
    if (nullptr != m_aboutListener)
    {
        PCSTR interfaces[] = { "com.dotMorten.PhilipsHueDSB.PhilipsHue.MainInterface" };
...
void MainInterfaceWatcher::OnAnnounce(
    _In_ PCSTR name,
    ...
    _In_ const alljoyn_msgarg aboutDataArg)
{
    ...
    if (alljoyn_aboutobjectdescription_hasinterface(objectDescription, "com.dotMorten.PhilipsHueDSB.PhilipsHue.MainInterface"))
    {
        AllJoynServiceInfo^ args = ref new AllJoynServiceInfo(
            AllJoynHelpers::MultibyteToPlatformString(name),
            AllJoynHelpers::GetObjectPath(objectDescription, "com.dotMorten.PhilipsHueDSB.PhilipsHue.MainInterface"),
            port);
        Added(this, args);
    }
...
void MainInterfaceWatcher::BusAttachmentStateChanged(_In_ AllJoynBusAttachment^ sender, _In_ AllJoynBusAttachmentStateChangedEventArgs^ args)
{
    if (args->State == AllJoynBusAttachmentState::Connected)
    {
        alljoyn_aboutlistener_callback callbacks = 
        {
            AllJoynHelpers::AnnounceHandler<MainInterfaceWatcher>
        };
...
void MainInterfaceWatcher::Start()
{
    ...
    int32 result = AllJoynHelpers::CreateInterfaces(m_busAttachment, c_MainInterfaceIntrospectionXml);
    ...
    m_busAttachmentStateChangedToken = m_busAttachment->StateChanged += ref new TypedEventHandler<AllJoynBusAttachment^, AllJoynBusAttachmentStateChangedEventArgs^>(this, &MainInterfaceWatcher::BusAttachmentStateChanged);
    m_busAttachment->Connect();
}

void MainInterfaceWatcher::Stop()
{
    StopInternal(AllJoynStatus::Ok);
}

void MainInterfaceWatcher::StopInternal(int32 status)
{
    UnregisterFromBus();
